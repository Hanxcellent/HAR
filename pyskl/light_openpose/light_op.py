...
from light_openpose.val import normalize, pad_width

net = PoseEstimationWithMobileNet()
checkpoint = torch.load('light_openpose/checkpoint_iter_370000.pth', map_location='cpu')
...
def infer_fast(net, img, net_input_height_size, stride, upsample_ratio,
    ...
    tensor_img = tensor_img.cpu()  # 这行下一行是10**-5级别
    net = net.cpu()
...
def estimate_pose(net, img):
    ...
    heatmaps, pafs, scale, pad = infer_fast(net, img, 256, stride, upsample_ratio)
    ...
    for n in range(len(pose_entries)):
        if len(pose_entries[n]) == 0:
            continue
        pose_keypoints = np.ones((num_keypoints, 3), dtype=np.float64) * -1
