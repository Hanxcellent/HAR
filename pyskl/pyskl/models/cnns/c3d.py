...
class C3D(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv1a = ConvModule(self.in_channels, self.base_channels, **c3d_conv_param)  # 17,32,56,56
        self.pool1 = nn.AvgPool3d(kernel_size=(1, 2, 2), stride=(1, 2, 2))

        self.conv2a = ConvModule(self.base_channels, self.base_channels * 2, **c3d_conv_param)  # 32,64,28,28
        self.pool2 = nn.AvgPool3d(kernel_size=pool_kernel, stride=pool_stride)

        self.conv3a = ConvModule(self.base_channels * 2, self.base_channels * 4, **c3d_conv_param)  # 64,128,14,14
        self.conv3b = ConvModule(self.base_channels * 4, self.base_channels * 4, **c3d_conv_param)  # 128,128,14,14
        self.pool3 = nn.AvgPool3d(kernel_size=pool_kernel, stride=pool_stride)

        self.conv4a = ConvModule(self.base_channels * 4, self.base_channels * 8, **c3d_conv_param)  # 128,256,7,7
        self.conv4b = ConvModule(self.base_channels * 8, self.base_channels * 8, **c3d_conv_param)  # 256,256,7,7
